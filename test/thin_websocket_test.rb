...
class Palmade::PuppetMaster::ThinConnection
  def send_data(data)
    (@data ||= [ ]).push(data)
  end
end

class ThinWebSocketTest < Test::Unit::TestCase
  def setup
  end

  def thin_conn_klass
    ::Palmade::PuppetMaster::ThinConnection
  end
  ...
  def test_websocket_mixin
    conn = thin_conn_klass.new(1)
    conn.extend(Palmade::PuppetMaster::ThinWebsocketConnection)
